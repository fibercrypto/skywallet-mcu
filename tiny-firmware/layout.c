...
void layoutDialog(const BITMAP* icon,
    const char* btnNo,
    const char* btnYes,
    const char* desc,
    const char* line1,
    ...
    const char* line6)
{
    ...
    if (desc) {
        ...
        if (btnYes || btnNo) { oledHLine(OLED_HEIGHT - 21); }
    } else {
        if (line5) oledDrawString(left, 4 * 9, line5, FONT_STANDARD);
        if (line6) oledDrawString(left, 5 * 9, line6, FONT_STANDARD);
        if (btnYes || btnNo) { oledHLine(OLED_HEIGHT - 13); }
    }
    if (btnNo) { layoutButtonNo(btnNo); }
    if (btnYes) { layoutButtonYes(btnYes); }
    oledRefresh();
}
...
void layoutProgress(const char* desc, int permil)
{
    ...
    if (desc) { oledDrawStringCenter(OLED_HEIGHT - 16, desc, FONT_STANDARD); }
    oledRefresh();
}
