...
void setup(void)
{
    ...
    // compatibility with other Cortex processors" of the ARM Cortex-M3
    // Processor Technical Reference Manual. According to section 4.4.2
    // and 4.4.7 of the "STM32F10xxx/20xxx/21xxx/L1xxxx Cortex-M3 programming
    // manual", STM32F2 series MCUs are r2p0 and always have this bit set on
    // reset already.
    ...
    // Section 20.3.1 we don't use the first random number generated after
    // setting the RNGEN bit in setup
    random32();
...
void setupApp(void)
{
    ...
    // that had enabled them in RNG control register (the RNG interrupt was
    // never enabled in the NVIC)
    RNG_CR &= ~RNG_CR_IE;
    ...
    // manual - Section 20.3.1: "Each subsequent generated random number has to
...
void mpu_config(void)
{
    ...
    // Firmware (0x08010000 - 0x0807FFFF, 64 + 3 * 128 KiB = 64 + 128 + 256 KiB
    // = 448 KiB, read-only)
    MPU_RBAR =
        FLASH_BASE | 0x10000 | MPU_RBAR_VALID | (2 << MPU_RBAR_REGION_LSB);
    MPU_RASR = MPU_RASR_ENABLE | MPU_RASR_ATTR_FLASH | MPU_RASR_SIZE_64KB |
               MPU_RASR_ATTR_AP_PRO_URO;
    MPU_RBAR =
        FLASH_BASE | 0x20000 | MPU_RBAR_VALID | (3 << MPU_RBAR_REGION_LSB);
    MPU_RASR = MPU_RASR_ENABLE | MPU_RASR_ATTR_FLASH | MPU_RASR_SIZE_128KB |
               MPU_RASR_ATTR_AP_PRO_URO;
    MPU_RBAR =
        FLASH_BASE | 0x40000 | MPU_RBAR_VALID | (4 << MPU_RBAR_REGION_LSB);
    MPU_RASR = MPU_RASR_ENABLE | MPU_RASR_ATTR_FLASH | MPU_RASR_SIZE_256KB |
               MPU_RASR_ATTR_AP_PRO_URO;
